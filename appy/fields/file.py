...
import os, os.path, time, mimetypes, shutil, base64
...
def getShownSize(size):
    ...
    if size < 1024: return '%s Mb' % sutils.formatNumber(size, precision=1)
    size = size / 1024.0 # This is the size, in Gb
    return '%s Gb' % sutils.formatNumber(size, precision=1)
...
class FileInfo:
    ...
    def __init__(self, fsPath, inDb=True, uploadName=None):
        '''p_fsPath is the path of the file on disk.
           ...
           - If p_inDb is False, this FileInfo is a simple temporary object
             ...
             database-controlled files (like methods removeFile, writeFile or
             copyFile).'''
        self.fsPath = fsPath
    ...
    def getFilePath(self, obj, checkTemp=True):
        ...
        if not os.path.exists(r) and checkTemp:
