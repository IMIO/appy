...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
def subUnzip(folder, zipName, odf=False):
    ...
    unzip(absZipName, subFolder, odf=odf, unzipSubZips=True)
    # Delete p_zipName
        ...
        if fileName:
            ...
            if odf and not folderName:
                ...
                if fileName in odfInnerFiles:
                    r[fileName] = fileContent if asBytes \
                                              else fileContent.decode()
            f.write(fileContent)
            f.close()
            ...
            if unzipSubZips and fileName.endswith('.zip'):
                subUnzip(fullFolderName, fileName, odf=odf)
    zipFile.close()
    return r
