...
class HtmlDiff:
    ...
    def __init__(self, old, new,
        ...
        # p_old and p_new are strings (or possibly None) containing chunks of
        # XHTML code.
        self.old = (old or '').strip()
        self.new = (new or '').strip()
...
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
class ColoredDiff:
    '''Produce a XHTML version of the output of a the Unix diff (or derivates,
       like git diff).'''
    ...
    def __init__(self, rdiff, splitAbove=100):
        ...
        self.rdiff = rdiff
        # Visually split a line if above this number of chars
        self.splitAbove = splitAbove

    def splitLine(self, line, lmax=None):
        '''Split this p_line into several sub-lines, each one being at most
           p_lmax, or, if None, p_self.splitAbove chars.'''
        # Return the p_line unchanged if shorter enough
        max = lmax or self.splitAbove
    ...
    def run(self):
        '''Compute and returns a XHTML chunk, as a string, containing the
           colored version of p_self.rdiff.'''
        r = []
        esc = Escape.xhtml
        # m_splitLine will split a line into chunks of max p_self.splitAbove
        # chars, but is coded as a recursive method. If the line is too long,
        # maximum recursion depth could be reached. In that case, the line is
        # truncated.
        ...
        for line in self.rdiff.split('\n'):
            ...
            line = esc(line)
            # Compute the CSS class to potentially apply to this v_line
            if line.startswith('+'):
                css = 'a_d_d'
