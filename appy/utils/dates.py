...
class Date:
    ...
    @classmethod
    def singleDay(class_, start, end, days=1):
        '''Does period [p_start, p_end] (both being DateTime objects) span a
           single day? If p_end occurs one day after p_start, but interval
           [p_start, p_end] is less than this number of p_days, the answer is
           yes, too.'''
        # The objective is to detect if an event, organized from p_start to
        # p_end, is a single event or represents multiple events occuring on a
        # series of consecutive days. For example, an event starting at 20:00
        # and ending at 02:00 will be considered being a single event. An event
        # starting one day at 08:00 and ending the day after, at 18:00 PM, will
        # be considered to include 2 sub-events, one for each day.
    ...
    @classmethod
    def periodsIntersect(class_, start1, end1, start2, end2, strict=True):
        ...
        if strict:
            disjoint = start1 > end2 or start2 > end1
        else:
            disjoint = start1 >= end2 or start2 >= end1
        return not disjoint
