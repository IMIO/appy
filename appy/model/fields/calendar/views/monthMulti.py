...
class MonthMulti(Month):
    ...
    def getCellTitle(self, events, sep='&#013;'):
    ...
    def getCellStyle(self, events, title):
        '''Gets the cell style to apply to the cell corresponding to p_date'''
        if not events: return
        # If a p_title is defined, set a specific cursor
        r = 'cursor:help;' if title else ''
        if len(events) > 1:
            # Return a special background indicating that several events are
            # hidden behing this cell.
            r = f'{r}{self.cellGradient}'
        else:
            r = f'{r}{events[0].getStyles()}'
        return r
    ...
    def getCellSelectParams(self, date, content):
        ...
        return f' onclick="onCell(this,\'{dateS}\')"', ' class="calicon"'
    ...
    def getCell(self, c, date, actions):
        ...
        style = style or self.getCellStyle(events, title)
        # Define its content
        disableSelect = False
        if not content:
