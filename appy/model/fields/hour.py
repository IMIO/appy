...
class Hour(Field):
    ...
    @classmethod
    def addMinutes(class_, value, minutes, toRound=True):
        ...
        # If m_minutes is a float number, round it if p_toRound is True
        if toRound and isinstance(minutes, float):
    ...
    @classmethod
    def formatDuration(class_, minutes, sep='h', no='-', toRound=True):
        '''Returns a formatted version of this number of p_minutes'''
        if minutes is None: return no
        # If p_toRound is True, the number of minutes, if expressed as a float,
        # is rounded.
        if toRound and isinstance(minutes, float):
            minutes = round(minutes)
        if minutes < 0:
