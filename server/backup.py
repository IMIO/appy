...
class Backup:
    ...
    def copyDatabase(self):
        ...
        if not destFolder.is_dir():
            # p_exist_ok is True. That way, if a tool like autofs automounts the
            # folder in the meanwhile, no exception is raised.
        ...
        for ext in exts:
            ...
            for fileName in tempFolder.glob('*.%s' % ext):
                try:
                    ...
                    # The process may not be allowed to perform such deletions
                    undeleted += 1
            counts[ext] = count
        # Log deletions
